...
struct DishGlyphCatalogTests {
    ...
    @Test func everySeedRecipeIsExplicitlyMapped() {
        ...
        // 개수는 하한만 본다 — 정확한 수를 단언하면 시드에 레시피를 한 줄 더하는 것만으로 CI가
        // 빨개진다(회귀 방어가 아니라 변경 억제). 매핑 누락은 아래 루프가 항목별로 잡는다.
        #expect(!recipes.isEmpty, "시드가 로드되지 않았다 — 매핑 계약을 검증할 수 없다")
        for r in recipes {
...
/// 어느 쪽으로 가는지가 흔들리면 티켓의 얼굴이 바뀐다(비빔밥이 시금치 잎으로 뜨던 회귀).
struct RecipeHeroIconTests {

    /// ① 시드는 전부 요리 그림 — 티켓 메뉴명 옆에서 재료 글리프가 뜨면 안 된다.
    ...
    @Test func heroIconDivergesFromTheCatalogOnlyWhereIntended() {
        let samples: [Recipe] = seedRecipesForTests() + [
            Recipe.userRecipe(name: "김밥", ingredientNames: ["김", "밥"], minutes: 20),
            Recipe.userRecipe(name: "된장찌개", ingredientNames: ["두부"], minutes: 20),
            Recipe.userRecipe(name: "Halmeoni Special", ingredientNames: ["토마토"], minutes: 20),
        ]
        for r in samples {
            switch r.heroIcon {
            case .dish(let look):
...
/// 공유 카드 재렌더 키(`CookingStepsView.ShareCardKey`) 계약 — 키는 **카드에 인쇄되는 값 전부**를
/// 담아야 한다. 조리 중 레시피가 삭제되면 화면 아이콘(`heroIcon(for:)`)은 `RecipeHeroIcon.session`
/// 폴백으로 갈아타는데, 세션 스냅샷이라 삭제와 무관하게 불변인 `recipeName`만으로 재렌더를
/// 트리거하면 화면과 공유 이미지가 서로 다른 요리를 보인다.
struct CookingShareCardKeyTests {

    /// 카드 입력과 키를 1:1로 묶는 헬퍼 — 실제 호출부(`shareCardKey(for:)`)와 같은 조합이다.
    private func key(name: String, icon: RecipeHeroIcon,
                     ingredientNames: [String] = ["토마토"], minutes: Int? = 20,
                     count: Int = 1) -> CookingStepsView.ShareCardKey {
        CookingStepsView.ShareCardKey(recipeName: name, ingredientNames: ingredientNames,
                                      minutes: minutes, count: count, icon: icon)
    }

    /// 실제 시나리오 — ④(재료 글리프) 히어로를 가진 커스텀 레시피가 조리 중 삭제되면
    /// `heroIcon(for:)`는 라이브 조회(`recipe.heroIcon`)에서 세션 폴백으로 갈아탄다. 두 아이콘이
    /// 실제로 다른지(전환 전제)부터 확인한 뒤, 나머지가 전부 같은 두 키가 갈리는지 잠근다.
    @Test func recipeDeletionTransitionChangesTheShareCardKey() {
        let recipeName = "Halmeoni Special"
        let recipe = Recipe.userRecipe(name: recipeName, ingredientNames: ["토마토", "양파"], minutes: 20)
        ...
        #expect(beforeDeletion != afterDeletion,
                "전환 전제가 깨졌다 — 이 레시피는 삭제돼도 같은 그림이라 시나리오를 재현 못한다")

        #expect(key(name: recipeName, icon: beforeDeletion) != key(name: recipeName, icon: afterDeletion),
                "조리 중 레시피 삭제로 화면 아이콘이 바뀌었는데 공유 카드 키가 그대로다 — 공유 이미지가 스테일로 남는다")
    }

    /// "김밥"·"참치김밥"은 둘 다 `.food(.gimbap)`으로 해석되지만(§13.7 ②) 이름이 다른 새 세션이므로
    /// 키가 달라 재렌더돼야 한다 — 아이콘 단독으로 키를 좁히는 회귀를 막는 잠금이다.
    @Test func sameHeroIconDifferentRecipeNameGetsDifferentKeys() {
        let gimbap = Recipe.userRecipe(name: "김밥", ingredientNames: ["김", "밥", "계란", "당근"], minutes: 20)
        let tunaGimbap = Recipe.userRecipe(name: "참치김밥", ingredientNames: ["참치", "김"], minutes: 15)
        #expect(gimbap.heroIcon == .food(.gimbap))
        #expect(tunaGimbap.heroIcon == .food(.gimbap))

        #expect(key(name: "김밥", icon: gimbap.heroIcon) != key(name: "참치김밥", icon: tunaGimbap.heroIcon),
                "아이콘이 같다고 키까지 같으면 참치김밥으로 새 세션을 열어도 옛 김밥 공유 이미지가 남는다")
    }
    ...
    @Test func deletionThatKeepsTheSamePictureKeepsTheSameKey() {
        let recipeName = "김밥"
        let recipe = Recipe.userRecipe(name: recipeName, ingredientNames: ["김", "밥", "계란", "당근"], minutes: 20)
        ...
        #expect(key(name: recipeName, icon: beforeDeletion) == key(name: recipeName, icon: afterDeletion),
                "그림이 같은데 키가 달라 불필요한 재렌더가 발생한다")
    }

    /// 재료 이름은 세션 스냅샷이 아니라 **라이브 재고**에서 파생된다 — 조리 중 예약 재료를 버리면
    /// 화면은 갱신되는데 키가 안 움직이면 공유 이미지만 옛 이름을 든 채 남는다.
    @Test func ingredientListChangeChangesTheShareCardKey() {
        let icon = RecipeHeroIcon.session(name: "Halmeoni Special", id: nil)
        #expect(key(name: "x", icon: icon, ingredientNames: ["토마토", "양파"], count: 2)
                != key(name: "x", icon: icon, ingredientNames: ["토마토"], count: 1),
                "예약 재료가 사라졌는데 공유 카드 키가 그대로다")
    }
}
...
struct ShareCardRenderTests {
    ...
    static func raster(recipe: Recipe, icon: RecipeHeroIcon) -> [UInt8]? {
        let card = RecipeShareCard(recipeName: recipe.displayName,
