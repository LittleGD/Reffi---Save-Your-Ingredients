...
- **부딪히면 달그락 — 재료별 촉감(CoreHaptics).** 햅틱은 스쿼시와 **다른 축**을 쓴다: 눌림은 질량으로 나눈 Δv(같은 높이서 떨어진 무거운/가벼운 칩이 같게 눌려야 한다), 촉감은 **질량으로 나누지 않은 생 임펄스**(운동량 전달이 곧 체감 크기 — 소고기가 잎사귀보다 묵직해야 한다). 충돌마다 `.hapticTransient` 하나를 치되 **세 관문**을 모두 통과한 것만 발화한다: ① 임펄스 **20 미만** 무시(구르는 중의 잔접촉까지 걸러낸다 — v1.0 (2)의 하한 6은 잔접촉 대부분을 통과시켜 "움직이면 그냥 일정한 진동"이 됐다) ② 전역 최소 간격 **90ms**(= 11Hz 상한 — 22Hz는 개별 '달그락'이 뭉개져 캐리어 진동처럼 들렸다. 달그락은 리듬이 들려야 달그락이다) ③ 같은 쌍 쿨다운 **260ms**(두 재료가 비빌 때의 연타 방지, 쌍 테이블 상한 64). 세기는 임펄스 20에서 `0.18`로 시작해 **90**에서 `1.0`으로 포화하고, 물성 클래스의 세기 배율(0.42~1.0)을 곱한다. **날카로움은 물성이 정한다** — `0.10`(두부·밥, 뭉툭한 '퍽') ~ `0.95`(캔·병, 쨍한 '클링'). 칩끼리면 **무거운 쪽**이 촉감을 주도한다. **§7.6의 의미별 햅틱 매핑(판정·성공·파괴)은 그대로다** — 이건 그 옆에 새로 놓는 **물리 질감 계열**이고, `.sensoryFeedback`은 세기·날카로움 파라미터가 없고 trigger 변경마다 물리 필드까지 다시 그려 여기엔 쓸 수 없어 CoreHaptics를 직접 쓴다. 엔진은 **씬이 보이는 동안만** 켠다(가려지면 내리고 쿨다운 테이블도 비운다). **Reduce Motion에서도 햅틱은 살아 있다** — 그건 시각 배려지 촉각 배려가 아니다(§7.4). 햅틱 하드웨어가 없으면(시뮬레이터) 조용히 무시한다.
