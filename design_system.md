...
| (참고) ink2 on receipt | ≥4.5 | 7.77 | 5.58 |
| (참고) muted on receipt | ≥3.0 | 4.46 | 3.04 |
...
- **부딪히면 달그락 — 재료별 촉감(CoreHaptics).** 햅틱은 스쿼시와 **다른 축**을 쓴다: 눌림은 질량으로 나눈 Δv(같은 높이서 떨어진 무거운/가벼운 칩이 같게 눌려야 한다), 촉감은 **질량으로 나누지 않은 생 임펄스**(운동량 전달이 곧 체감 크기 — 소고기가 잎사귀보다 묵직해야 한다). 충돌마다 촉감을 하나 치되 **세 관문**을 모두 통과한 것만 발화한다: ① 임펄스 **20 미만** 무시(구르는 중의 잔접촉까지 걸러낸다 — v1.0 (2)의 하한 6은 잔접촉 대부분을 통과시켜 "움직이면 그냥 일정한 진동"이 됐다) ② 전역 최소 간격 **70~110ms**(중심 90ms ± 지터 20ms — 평균 ≈11Hz. 22Hz는 개별 '달그락'이 뭉개져 캐리어 진동처럼 들렸고, 반대로 **정확히** 90ms는 격렬히 흔드는 동안 발화가 11Hz 격자에 딱 맞아 손이 충돌이 아니라 **기계적 클럭**을 읽었다 — 실기기 3차 "햅틱이 고정된 느낌"의 절반이 이 격자다. 지터는 난수가 아니라 **발화 순번 해시**라 같은 충돌 시퀀스는 실행마다 같은 리듬이다) ③ 같은 쌍 쿨다운 **260ms**(두 재료가 비빌 때의 연타 방지, 쌍 테이블 상한 64).
  ...
  - **감쇠 꼬리 — 상위 1/3에만.** 정규화 세기 `0.67` 이상이면 transient 뒤 12ms에 **40~70ms** `.hapticContinuous`를 붙이고 `CHHapticParameterCurve`로 세기를 0까지 끌어내린다('툭' 뒤의 잔향). 꼬리는 본 타격의 **0.42배 세기 · 0.28 낮은 날카로움**이다 — 같은 세기면 두 번 친 것으로, 같은 날카로움이면 두 번째 '클링'으로 들린다. **모든 충돌에 붙이면 잔향이 겹쳐 이 기능이 처음 고치려던 '웅웅'으로 되돌아간다.** 두 이벤트는 **한 패턴**으로 낸다(플레이어를 둘로 나누면 시작 시각이 프레임 지터만큼 어긋나 꼬리가 타격보다 먼저 도착한다).
...
**History 패인의 맨 위는 헤드라인 `Kitchen ledger`, 그 아래가 이번 주 히어로다(2026-08, 24차).** 30일 정산서 위에 **종이 고리 + 요일 블롭 일곱 + 캡션 한 줄**이 전면 블리드 밴드로 선다 — 규격은 §13.10. 패인은 여전히 **한 덩이 스크롤**이라 헤드라인과 히어로는 스크롤과 함께 걷히고 탭 행만 남는다(위 고정 크롬 규칙 그대로, s5 간격 불변).
...
1. 사용 가능한 임박 재료를 백그라운드 계산 → 추천. **부족 재료가 3개 이상인 레시피는 원칙적으로 덱에 올리지 않는다**(2개까지는 그냥 추천, `RecipeRecommender.maxMissingForRecommendation`). 재료를 셋씩 사와야 하는 티켓은 "지금 냉장고를 비우는 요리"가 아니라 장보기 계획이라, 오늘 상해가는 재료를 쓰는 티켓을 밀어낼 자격이 없다. 0~2개는 그대로 추천하고 그 부족분은 Short 줄과 그 옆 "Add to list" 알약이 여는 **담기 3단 팝업**(고르기 체크리스트 → 담김 알림 → 이동 질문)이 처리한다(§13.5 ⑨).
...
- **구조(위→아래)** — ① **헤더**: 제목(`subhead`) + 무낭비 스트릭 `DDayStamp`(`DAY N`, `fresh-dark`, 10pt) ② **정산 두 행**: 라벨(`checklistItem`) + 건수(`reffiNum(.body)`, tabular이라 자릿수가 세로로 맞는다) ③ **낭비율 행**: 라벨 + **낭비율 도장**(`DDayStamp` 15pt, 잉크는 구간색 — §2.4 정렬된 재사용 예외) ④ **자주 버린 재료**: 캡션 라벨 + `PaperSilhouette`(`ReffiFoodIcon.row`) + 이름 + 횟수 ⑤ **마감**: 상호(`monoEyebrow`) + 영수증 번호(`reffiNum(.meta)`).
...
> History 패인의 첫 화면을 **한 장의 큰 그림**으로 연다(2026-08). 그 그림 위에는 패인 헤드라인 `Kitchen ledger` 한 줄만 선다(24차, §13.5) — 히어로는 여전히 패인의 **첫 블록**이고 헤드라인은 그것을 이름 붙이는 줄이라 밴드 밖 캔버스에 앉는다(밴드 안으로 넣으면 고리의 제목으로 읽혀 30일 정산서까지 덮는 이름 노릇을 못 한다). 레퍼런스(Calm 홈 = 전면 히어로 + 가운데 링 + 요일 동그라미)의 **구조만** 가져오고 광택은 가져오지 않는다: 링은 매끈한 진행 링이 아니라 **가위로 오린 다각 고리**고, 요일 동그라미는 §13.1의 9각 `PaperBlob`이며, 배경은 사진이 아니라 **내 재료 실루엣 더미**다.

- **창은 이번 주다(30일이 아니다).** 바로 아래 요일 행이 이번 주 7일이라, 고리만 30일이면 한 블록 안에서 서로 다른 두 책을 읽게 된다 — 옛 도넛이 링과 가운데 숫자에 다른 분모를 놓아 실패한 지점이 정확히 그것이다(§13.9). 지금은 **요일 칸 일곱의 합이 곧 고리의 분자**라 화면에서 눈으로 검산된다. 30일 수치는 아래 정산서가 자기 라벨("Tally · past 30 days")과 함께 계속 말한다. 두 창이 한 화면에 공존하는 대신, **각자 자기 창의 이름을 달고 있다**("eaten this week" / "past 30 days").
- **값은 먹은 비율, 색은 낭비율의 색.** 고리가 보여 주는 수치는 "이번 주에 처리한 것 중 안 버린 비율"이지만 잉크는 `HistoryContent.rateColor(낭비율)`에서 온다 — 앱 전체에서 초록은 "덜 버렸다"이고 그 임계값의 단일 공급원이 그 함수다. 여기서 뒤집지 않으면 잘한 주가 빨개진다.
- **`PaperRing` / `PaperRingArc`(신규 프리미티브).** 12시에서 시계방향으로 도는 **직선 변 폴리라인 띠** — 지름 **156** · 두께 **18**(안지름 120이 가운데 세 줄을 받는 폭이다). 트랙(`sub` 면)과 값(구간색 면)은 **종이 두 장**이고, 둘 다 평평한 토큰 색 + `PaperGrain` + `paperEdge`(값 띠는 `--paper-edge-onfill`)로 끝난다. **그라데이션·글로우·그림자 없음.** 기본 18각(20°마다 꺾여 지름 156pt에서 변 하나가 27pt, 원 대비 처짐 1.2pt)이고, 정점 반지름 지터는 **한 바퀴에 정확히 맞물리는 저주파 표**다 — 이웃 칸끼리 튀는 표를 쓰면 정점마다 뿔이 서서 톱날이 된다(첫 캡처의 12시 정점이 이웃보다 2.7% 튀었다). 두 가지 규율을 셰이프가 스스로 지킨다: ① **비율 0은 도형을 만들지 않는다**(폭 0 조각의 잉크 점 금지) ② **꽉 찬 고리는 닫힌 서브패스 둘**(바깥 시계·안쪽 반시계, non-zero로 가운데가 뚫린다) — 한 폴리곤으로 이으면 12시에 폭 0의 반지름 슬릿이 윤곽에 남아 `paperEdge` 스트로크가 거기에 실선을 그린다(첫 캡처에서 세로 이음매로 보였다). 잘린 단면은 **부분 호의 양 끝에만** 있다. **고리 안은 비율 + "eaten this week" + 분자·분모("N of M") 세 줄**이고 좌우 여백은 **s1**이다 — 안지름 120에서 s3씩 물리면 캡션이 96pt에 갇혀 기본 글자 크기에서도 축소된다(여백은 고리 안쪽 곡률이 이미 준다). 분자·분모를 눈에 보이게 두는 이유는 비율만 세우면 히어로의 가장 큰 숫자가 사용자의 행동에 반응하지 않는 구간이 생기기 때문이다(이번 주에 버린 게 없으면 뭘 더 먹어도 100% 고정) — 이 줄은 판정마다 반드시 움직이고 동시에 표본 크기를 드러내, 아래 30일 정산서의 낭비율과 나란히 놓였을 때의 모순감도 함께 푼다.
- **요일 행 — 오늘만 잉크 솔리드.** 7칸이 균등 폭으로 서고 각 칸은 머리글자(로케일의 `veryShortStandaloneWeekdaySymbols`) + 9각 `PaperBlob`(한 변 **38**, 칸 사이 s1) + 그날 먹은 개수(`reffiNum(.body)`)다. 지나간 날 = `paper` 면 + `paperEdge`, **오늘 = 잉크 솔리드 + `PaperGrain` + `paperEdgeOnFill`**(숫자는 `canvas`로 반전), 아직 오지 않은 날 = `sub` 0.55 면에 숫자 없음. **위 카테고리 칩의 "잉크 솔리드 금지"와 충돌하지 않는다**: 그 규칙은 *조작 상태*가 조작 대상보다 무거워지는 것을 막으려는 것이고, 오늘 칸은 조작 상태가 아니라 이 행에서 **어디가 지금인지**를 정하는 유일한 기준점이다(그것 없이는 어느 쪽이 지나간 날이고 어느 쪽이 앞으로 올 날인지 읽히지 않는다) — 탭 알약이 패인 행에서 하는 일과 같다. 게다가 이 행은 눌리지 않아 무거운 표시가 컨트롤로 오독될 여지도 없다.
...
- **빈 창** — 이번 주에 처리한 게 0건이면 숫자를 세우지 않고 고리 안에 안내 한 줄만 둔다("Nothing this week"). 0/0을 0%로 내면 "이번 주에 다 버렸다"는 없는 판정이 뜨고, 나눗셈을 그대로 두면 NaN이 된다(§13.9의 "빈 이력" 규칙과 같은 결).
- **`PaperGlyphPile`(신규) — 배경은 내 재료다.** 냉장고 재고 → 이력 순으로 글리프를 중복 없이 모아(둘 다 비면 고정 세트) **흔들린 격자**(5 × 4)에 결정적 시드로 흩뿌리고, `ImageRenderer`로 **한 장의 이미지로 구워** 흐리게 깐다. 완전 난수 산포는 스무 개로 뭉침과 빈 구멍을 만들고, 살아 있는 `Canvas` 스무 개는 스크롤 프레임마다 벡터 드로우 스무 번이라 **배경 장식이 앞의 콘텐츠보다 비싸진다**. 굽는 그림은 스킴에 무관하다(실루엣 팔레트는 재료의 실제 색 = 고정값) — 다시 굽는 조건은 글리프 구성과 실측 크기뿐이다. 장식이므로 `accessibilityHidden`.
- **스크림은 바탕과 같은 토큰.** 밴드 바탕 `paperPass` 위에 더미, 그 위에 다시 `paperPass` 반투명. 다른 색을 얹으면 라이트/다크 중 한쪽에서 밴드가 두 톤으로 갈린다. **실루엣이 바탕에서 벗어나는 폭 = 더미 농도 × (1 − 스크림)이고 현재 0.36 × 0.45 ≈ 0.16이다.** 첫 캡처를 실측했더니 0.225에서 다크 모드의 밝은 실루엣(달걀·우유) 위 캡션이 **4.14:1**까지 떨어졌다 — 14pt Medium은 WCAG 큰 글자가 아니라 4.5:1이 기준선이다(§2.6). 0.16으로 조인 뒤 최악 대비는 다크 4.80:1 / 라이트 5.58:1(실측).
- **전면 블리드.** 히어로는 카드가 아니라 패인이 앉은 **바닥 면**이라 좌우 마진에 갇히면 카드 한 장으로 오해된다 — 부모 마진을 `-ReffiGrid.margin`으로 되돌린다(카테고리 칩 행이 쓰던 관용구 그대로).
- **집계는 `ConsumptionWeek`(순수 함수) 한 곳.** 뷰가 이력을 직접 훑으면 고리의 분모와 요일 칸이 조용히 갈린다. 주 시작은 `Calendar.firstWeekday`, 칸 배정은 **달력의 자정**이 한다 — 초 나눗셈(86400)은 서머타임이 낀 주에서 그날 밤 로그를 다음 날로 밀어낸다(유닛 테스트가 그 함정을 고정한다). 본문당 한 번만 계산해 고리·요일 행·접근성 라벨이 같은 값을 읽는다.
- **접근성** — 고리는 숫자 하나가 아니라 "무엇의 몇 퍼센트인가"로 읽는다("Eaten this week: 80 percent, 4 of 5 items"), 빈 창이면 빈 창이라고 말한다. 요일 칸은 각각 하나의 요소이고 이름 + 개수로 읽는다("Monday, 3 eaten" / 오늘은 "Today, 3 eaten" / 앞으로 올 날은 "Saturday, still to come").
