...
**탭 행과 타이틀은 스크롤 밖 고정 크롬이다**(타이틀과 s3, 아래 패인과 s4). 타이틀 옆에는 **재고 수 캡션**이 붙는다 — 화면엔 "· 9"(`caption`·`ink2`, `firstTextBaseline` 정렬)만 두고 보조기술엔 "9 in stock"으로 읽힌다(시각 축약은 접근성 라벨로 복원한다). 이 숫자는 **필터 이전의 전체 재고**라 카테고리를 좁혀도 변하지 않는다 — 패인을 오가는 조작이라 항상 같은 자리에 있어야 하고, 스크롤과 함께 사라지면 "지금 어느 탭인가"라는 유일한 표시를 잃는다. 패인 셋은 각자 스크롤하고, **배경 accent도 탭을 따라간다**(재고 = 첫 재료의 신선도색 · To buy = blue · History = 낭비율색) — 옛 커버 둘이 갖고 있던 색을 그대로 물려받아 전환이 배경에서도 읽힌다.
