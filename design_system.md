...
- [ ] 영수증 종이를 새로 그릴 때 톱니를 `ReffiTooth`로 지정했는가 — 숫자 리터럴 금지(§13.8)
...
- **톱니 토큰** `ReffiTooth` — `chip` 6(홈 미니 스트립·온보딩 소품처럼 폭이 좁은 조각) · `card` 7(목록·시트·이력의 영수증 카드) · `ticket` 9(오더·조리·공유 티켓). `ReceiptShape(tooth:)`의 기본값도 `ticket`이다. 숫자 리터럴을 호출부에 적지 않는다 — 종이 폭이 좁을수록 톱니를 줄여야 잘게 보이지 않고, 그 판단은 표면이 아니라 토큰이 한다.
