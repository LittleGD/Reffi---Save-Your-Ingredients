...
1. 사용 가능한 임박 재료를 백그라운드 계산 → 추천. **부족 재료가 3개 이상인 레시피는 덱에 올리지 않는다**(2개까지는 추천, `RecipeRecommender.maxMissingForRecommendation`). 재료를 셋씩 사와야 하는 티켓은 "지금 냉장고를 비우는 요리"가 아니라 장보기 계획이라, 오늘 상해가는 재료를 쓰는 티켓을 밀어낼 자격이 없다. 0~2개는 그대로 추천하고 그 부족분은 Short 줄과 "Add to To buy" 칩이 처리한다(§13.5 ⑩). 이 필터로 덱이 통째로 빌 수 있으므로, 재고는 있는데 후보가 0인 경우의 빈 상태 카피는 **재료가 모자라다는 원인과 장보기라는 행동**을 함께 말한다(이름 매칭 실패만 말하던 옛 문구를 교체).
2. 재료가 **위에서 진짜 물리로 떨어져 충돌·바운스하며 쌓여 그대로 남는다**(SpriteKit). 사라지지 않고, 끌어서 던질 수 있다.
2-1. **정지 상태의 필드 높이는 재료 수로 캡한다** — ⌈n/3⌉행 × 96 + 28pt(3열로 눕는 실측 피치), 캡한 필드는 남는 공백을 **위아래로 갈라** 더미를 광학 중앙에 앉힌다. 필드를 화면 끝까지 늘리면 중력에 눕는 더미는 바닥(뱃지 행 바로 위)에 붙고 위쪽 여유가 통째로 빈 띠로 남아, 미션 문구·배너와 그것이 가리키는 재료 사이가 뷰포트의 4분의 1로 벌어진다(감사 R3-4: 실측 170pt → 캡 후 123pt). **낙하 드라마는 이 캡과 무관하다** — 스폰 천장은 씬 바깥 절대 좌표(`size.height + 700`)라 낙하 높이·스태거가 그대로다. 재료가 없을 땐 캡하지 않는다(빈 작업대의 카피·＋추가는 화면 중앙 정렬 그대로).
...
### 14.7 종이 다이얼로그 — `PaperDialog`

앱 안에서 **알리거나 묻는** 팝업은 시스템 알림이 아니라 종이 다이얼로그로 띄운다(2026-08 결정). 크림 종이·손그림 테두리·종이컷 버튼으로 이뤄진 화면 한복판에 iOS 기본 알림이 뜨면 그 순간만 다른 앱이 되기 때문이다. 판정 커버(`DecisionCover`, §13.6 B)가 이미 쓰던 문법을 컴포넌트로 뽑은 것이다.

**구성**: `scrim` 딤(§12 토큰, 탭 동작은 아래) + `PaperRect(cornerRadius: xl)` 크림 종이 카드(`paper` 면 + `paperEdge` + `reffiShadow1`) + 제목(`heading`) + 선택적 메시지(`body`/`ink2`) + **종이컷 버튼 한 줄**(`PaperButton` 그대로, 새 버튼 어휘를 만들지 않는다). 카드 최대 폭 420pt(넓은 화면에서 한 줄이 길어지지 않게), 좌우 여백 `s6`.

**셰이프 규칙**: 다이얼로그는 **`ReceiptShape`(톱니 영수증)를 쓰지 않는다**. 티켓 덱 위에 뜨는 팝업이 톱니를 두르면 "티켓이 한 장 더 나온 것"으로 읽힌다. 다이얼로그는 티켓이 아니라 묻는 종이다.

**버튼 배치**: 취소(중립 `secondary`)가 왼쪽, 실행(파랑 `primary`)이 오른쪽. 시스템 알림과 같은 손 방향이라 근육 기억이 깨지지 않는다. 알림형은 실행 하나만 두고 폭을 채운다. 세로 패딩 `s4` + `subhead` 한 줄이면 실측 높이가 44pt를 넘는다(§7.3).

**모션**: 딤은 `easeOut(dur2)`로 들고 나며, 카드는 `pop` 스프링으로 튀어 오른다(§7.5). Reduce Motion이면 둘 다 즉시 표시.

**딤(바깥) 탭은 호출부가 의식적으로 정한다** — 기본값은 **무시**다.
| 성격 | 바깥 탭 | 근거 |
|---|---|---|
| 알림형(확인 하나) | **무시** | 되돌릴 게 없는 통지라 바깥 탭으로 조용히 끝나면, 방금 무슨 일이 일어났는지도 못 보고 이어질 질문도 못 받는다. 나가는 길은 확인 하나뿐이다 |
| 질문형(취소 + 실행) | **취소와 같은 것** | 실수로 닫아도 잃는 게 없고, 취소와 다른 결과를 내면 실수로 닫은 사용자가 의도하지 않은 행동을 하게 된다 |

**접근성**: 컨테이너에 `.isModal` — 뜬 동안 뒤 화면이 VoiceOver에서 사라진다. 등장 시 포커스를 제목으로 옮기고(`@AccessibilityFocusState`), 제목엔 `.isHeader`. 이스케이프 제스처는 바깥 탭과 **같은 행동**(정해지지 않았으면 1차 행동)으로 매핑한다 — 흐름이 끝나지 않은 채 다이얼로그만 사라지는 길을 만들지 않기 위해서다.

**프레젠테이션이 아니라 오버레이**라는 점이 중요하다. 풀스크린 커버 **안에서** 시스템 알림을 띄우면 알림 해체 전환이 부모 커버의 닫기 요청과 겹쳐 UIKit이 그 요청을 삼킨다(실측: 티켓 덱의 담기 흐름에서 3회 중 1회 재현, 우회하려고 0.6초 지연을 넣어야 했다). 같은 뷰 트리 안의 오버레이는 전환 큐가 없어 그 경쟁이 성립하지 않고, 두 장을 이어 띄우는 흐름도 같은 업데이트에서 교대된다.

**사용례**(티켓 덱의 "Add to To buy", §13.5 ⑩): 담기 결과 알림("Added to To buy" / "Already on your To buy list", 확인 하나) → 이동 질문("View your To buy list?" + "You can open it later from the Fridge tab.", 취소/보기).
