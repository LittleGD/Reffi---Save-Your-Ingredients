...
- **덱이 살아 있을 때는 미커버 행.** 오늘 만료(`urgent`)인데 **어느 티켓의 `used`에도 없는** 재료가 있으면(`RecipeRecommender.uncoveredUrgent` — 뷰 밖에서 검증되는 순수 함수), 덱 **위**에 한 줄짜리 종이 행(`PaperRect` `radius-sm`, 최소 44pt)이 뜬다: "Nothing on these tickets uses …" + 오른쪽 컴팩트 영상 버튼(유튜브 아이콘 18pt, 히트 44pt, `urgent-dark`). 행은 카드 위에 겹치지 않고 **자기 자리를 가져간다** — 실측 높이를 카드 높이 예산에서 빼므로 큰 글씨에서도 티켓 머리를 덮지 않는다. `soon`·`fresh`는 세지 않는다(상시 표시되면 경고가 배경이 된다). 미커버가 없으면 행 자체를 그리지 않는다.
- 영상 URL 조립은 `RecipeVideoSearch` **한 곳**에서 한다 — 조리 화면의 "Open recipe videos" CTA(§13.6 4-1)와 같은 함수다. 표면마다 다시 쓰면 인코딩 규칙이 갈려 한쪽만 조용히 유튜브 홈으로 떨어진다.
