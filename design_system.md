...
- **신선도에 따라 시든다(Wilt) — 팔레트는 안 바뀌고 자세가 바뀐다.** 재료 정체성(hue)은 색-코딩되지 않지만, 소비기한이 다가올수록 눈에 보이게 시들어 촉각적 압박을 준다. `WiltStyle`(`PaperSilhouette.swift`)이 Canvas 그리기 이음매 한 곳에서 채도(saturate) × 명도(brightness) 색행렬과 **밑변 고정** 스쿼시·기울임(전단) 변환을 적용해, 52종 글리프 draw 함수 전부가 무수정으로 상속한다:
