...
**History 패인의 맨 위는 이번 주 히어로다(2026-08).** 30일 정산서 위에 **종이 고리 + 요일 블롭 일곱 + 캡션 한 줄**이 전면 블리드 밴드로 선다 — 규격은 §13.10. 패인은 여전히 **한 덩이 스크롤**이라 히어로는 스크롤과 함께 걷히고 탭 행만 남는다(위 고정 크롬 규칙 그대로, s5 간격 불변).
...
> History 패인의 첫 화면을 **한 장의 큰 그림**으로 연다(2026-08). 레퍼런스(Calm 홈 = 전면 히어로 + 가운데 링 + 요일 동그라미)의 **구조만** 가져오고 광택은 가져오지 않는다: 링은 매끈한 진행 링이 아니라 **가위로 오린 다각 고리**고, 요일 동그라미는 §13.1의 9각 `PaperBlob`이며, 배경은 사진이 아니라 **내 재료 실루엣 더미**다.
