...
**냉장고 요약·정렬(`FridgeView`)도 §13 문법을 공유한다.** "Fridge" 타이틀 **바로 아래**(재고 수·정렬 행보다 **앞**) **요약 두 버튼 한 줄** — 장보기 · 무낭비 리포트가 반쪽 폭으로 나란히 선다(`PaperCutRect` 종이컷 버튼 면 + 옅은 그레인 + `--shadow-1`). 페이저(`TabView`)와 점 인디케이터는 걷어냈다 — 한 장씩 스와이프해야 두 번째가 보이는 구조라 리포트가 발견되지 않았고, 둘 다 상시 보이면 스와이프도 인디케이터도 필요 없다. 반쪽 폭이라 카드 안은 세로로 쌓는다(아이콘·값 한 줄 + 제목). **제목은 말줄임하지 않는다** — 이 카드에서 제목은 유일한 목적지 이름이라 잘리면 어디로 가는 버튼인지가 사라진다. 반쪽 폭에서는 영문 "No-waste report"가 기본 글자 크기에서도 한 줄에 안 들어가므로(번들 폰트 실측 122.4pt vs iPhone SE 가용 119.5pt) **두 줄까지 허용**하고 축소(0.7)는 그 뒤 마지막 수단이다. 한쪽 제목만 두 줄이 되어도 **두 카드는 같은 높이로 선다** — 행이 가장 높은 카드에 맞고 두 카드가 그 높이를 채운다(높이가 갈리면 나란한 두 종이의 위아래 모서리가 어긋난다). 셰브론은 뺐다(인터랙션 커먼 룰 ⑩ — 셰브론은 시트 기표인데 이 둘은 풀스크린 커버를 연다). 상단이 "여기가 어디인가 → 지금 할 일 → 목록 조작"의 순서로 읽힌다(타이틀과 s3, 재고 행과는 s5). 스택 위 **정렬 칩**(현재 정렬 라벨을 상시 노출)과 **간편보기 원탭 토글**은 둘 다 `PaperRect` 면 + `paperEdge`, 히트 영역 최소 44pt(§7.3). 정렬 칩을 탭하면 스톡 `Menu`/`Picker` 팝업(흰 시스템 라운드 렉트) 대신 **앱 최초 커스텀 드롭다운 `PaperDropdown`**이 칩 바로 아래에 뜬다 — `PaperRect` 면 + `paperEdge` 헤어라인 + 옅은 `PaperGrain` + `--shadow-1`, 행마다 라벨(좌) + **선택 행에만 체크 글리프**(우, `blue-dark`), 행 사이 `DashedRule`(절취선), 최소 44pt 히트·`paperPress`. 트리거 칩 앵커 아래에 `overlayPreferenceValue`로 띄워 `ScrollView`에 클리핑되지 않고(zIndex `dropdown`), 딤 없는 투명 탭 캐처가 바깥 탭을 받아 닫는다(가벼운 드롭다운 — 모달 아님, `scrim` 없음), 진입 `--spring-pop`·이탈은 더 빠르게(§7.5). **"탭 → 옵션 목록"은 앱 전체에서 이 한 문법이다** — 편집 시트(재료·스캔 후보)의 단위·보관 선택도 스톡 `Menu`/`Picker`가 아니라 같은 `PaperDropdown`이고, 트리거는 현재 값을 상시 노출하는 종이 칩(`PaperDropdownTrigger`, 히트 44pt)이다. 시트 안에서는 시스템 팝오버와 달리 오버레이가 시트 밖으로 나갈 수 없으므로 **아래/위 여유를 재 뒤집고 높이를 캡**하며, 넘치면 팝업이 내부 스크롤한다(단위 10종). 시각 선택(`NotifyTimeSheet`)만 `.wheel`로 남는다 — 다이얼은 목록이 아니다. **간편보기(`FridgeCompactRow`)**는 틸트·겹침 없는 납작한 흰 영수증 한 조각 — 실루엣 + 이름 + 수량 + D-day(냉동이면 + FROZEN 도장)를 한 줄에 정렬해 훑어보기(스캔)에 최적화한다.
