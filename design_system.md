...
바로 아래 **카테고리 필터 칩 행은 잉크 솔리드 반전을 쓰지 않는다**(아래 문단) — 두 규칙이 어긋나 보이지만 같은 원리의 두 결과다: 필터는 콘텐츠(영수증 스택)에 **종속된 상태**라 표시가 콘텐츠보다 가벼워야 하고, 탭은 **이 화면이 지금 무엇인가**를 정하는 내비게이션이라 화면에서 가장 확실한 신호여야 한다. 두 행의 무게가 갈리는 것이 오히려 위계를 세운다(내비 > 필터).
...
**탭 행과 타이틀은 스크롤 밖 고정 크롬이다**(타이틀과 s3, 아래 패인과 s4) — 패인을 오가는 조작이라 항상 같은 자리에 있어야 하고, 스크롤과 함께 사라지면 "지금 어느 탭인가"라는 유일한 표시를 잃는다. 패인 셋은 각자 스크롤하고, **배경 accent도 탭을 따라간다**(재고 = 첫 재료의 신선도색 · To buy = blue · History = 낭비율색) — 옛 커버 둘이 갖고 있던 색을 그대로 물려받아 전환이 배경에서도 읽힌다.
...
**카테고리 필터 칩 행**(신규, 2026-08)은 헤더·요약 아래, 리스트 위에 가로 스크롤로 얹힌다 — 재고에 실제 있는 카테고리만 캐논 순서(Veg → Fruit → Dairy → Meat → Seafood → Protein → Bakery → Grain → Pantry → Other)로 노출하고, 맨 앞에 "All"이 붙는다. 정렬 칩과 같은 `PaperRect` 문법이되 **선택 상태는 굵은 잉크 단면**으로 표현한다 — 선택 = `sub` 면 + `ink` 55% 단면 **2pt**, 비선택 = `paper` 면 + 헤어라인 `paperEdge`(글자는 양쪽 다 `ink` + `ink2` 개수, 정렬 칩은 라벨을 상시 노출하는 방식이라 문법이 다르다 — 드롭다운의 체크 문법은 팝업 전용으로 남긴다). **`ink` 솔리드 반전은 쓰지 않는다** — 반전 칩은 그 화면에서 유일한 잉크(다크에선 유일한 크림) 면이 되어 필터 *상태*가 콘텐츠인 영수증 스택보다 무거워졌다. **선택 표시는 콘텐츠보다 가벼워야 한다**는 것이 이 칩 행의 규칙이다. 단일 선택, 재탭·"All" 탭으로 해제. **카테고리가 한 종류뿐이면 칩 행 자체를 그리지 않는다**(동작 없는 UI 금지). 세션 한정 `@State`(정렬과 달리 영속화하지 않음) — 필터링된 목록이 비면 자동으로 전체로 풀리고, 필터 밖으로 밀려난 펼침 카드는 접힌다(빈 화면에 가두지 않기 위한 안전망). 그룹 키는 저장된 `ingredient.category` 자유 문자열이 아니라 **`FoodGlyph.categoryLabel`**(글리프 파생, 항상 캐논 10종이라 전부 번역돼 있고 칩 집합이 안정적) — 레거시 자유 문자열("Meat · Beef" 등)도 한 칩으로 흡수된다(카드에 보이는 표시 텍스트는 그대로, 필터 키만 다르다). 히트 영역 최소 44pt(§7.3), 선택 칩엔 `.isSelected` 접근성 트레잇.
