...
- **착지가 통통 튄다.** `SKPhysicsContactDelegate`로 충돌 임펄스를 질량으로 나눈 근사 Δv(pt/s)를 재서, **30 이상**이면 눈에 보이는 스쿼시(가로 +3~6% / 세로 −3~6%, `--dur-2` 180ms, 눌림 35% ease-out + 복귀 65% ease-out, 배율은 항상 정확히 1로 복귀, 상한 **260**에서 최대 눌림). 다만 임계만으론 부족하다 — **착지인 접촉만 누른다**: 접촉 법선과 그 시점 중력 단위벡터의 `|dot|`이 **0.55를 넘을 때만** 스쿼시한다. 기울여 굴릴 때의 옆접촉은 법선이 중력에 수직(dot≈0)이라 Δv가 30을 넘어도 걸러지고, 바닥·더미 위 착지는 **기울인 중력에서도** 법선이 중력축과 나란해 그대로 통과한다(v1.0 (3) 실기기 피드백 "크기가 커졌다 작아지는 움찔거림" — 굴리는 내내 더미가 숨쉬듯 눌렸다). 여기에 **칩당 쿨다운 250ms**를 더한다 — 실행 중 재진입 가드는 애니메이션 길이(180ms)만 덮어 끝나자마자 다음 접촉이 또 눌렀다. 이 게이트는 **시각 축에만** 건다: 굴러 부딪히는 달그락 햅틱은 들리는 편이 맞다. Reduce Motion·유휴 상태·드래그 중인 칩은 스쿼시를 생략한다.
