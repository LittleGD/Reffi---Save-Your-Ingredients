...
**To buy·History 패인은 커버와 같은 뷰를 크롬 없이 얹는다.** 두 화면의 본문은 `ShoppingListContent`· `HistoryContent`로 떼어져 있고, 풀스크린 커버 형태(`ShoppingListView`·`HistoryView`)는 `CoverHeader`(§14.2)와 배경만 씌우는 얇은 래퍼로 남는다 — 두 표면이 같은 목록을 각자 그리면 규칙이 조용히 갈린다. 패인과 커버가 다른 건 **바닥 여백 하나**다: 냉장고 페이지엔 떠 있는 캡슐 네비가 있어 To buy의 하단 도킹 CTA("Add item")는 그 자리(`ReffiChrome.navReserve`)를 비우고, History 패인은 `navClearance`까지 스크롤 꼬리를 준다. 하단 크림 마스크(페이드 + 불투명 띠)는 **그냥 스크롤하는 두 패인(In stock·History)에만** 건다 — 카드가 캡슐 네비 유리 뒤에서 반쯤 읽히는 잔상을 막는 장치이고(History 타임라인에서 실측으로 재현됐다), To buy는 도킹 CTA가 이미 같은 자리에 불투명 면을 깔아서 마스크를 얹으면 그 버튼이 밑에 깔린다. 펼친 영수증(카드 탭 → 상세)은 탭 행까지 덮는 **전체 화면**이고 탭을 옮기면 선택이 풀린다 — 판정(Ate/Tossed)까지 한 화면에서 끝나야 하기 때문이다.
...
1. 사용 가능한 임박 재료를 백그라운드 계산 → 추천. **부족 재료가 3개 이상인 레시피는 원칙적으로 덱에 올리지 않는다**(2개까지는 그냥 추천, `RecipeRecommender.maxMissingForRecommendation`). 재료를 셋씩 사와야 하는 티켓은 "지금 냉장고를 비우는 요리"가 아니라 장보기 계획이라, 오늘 상해가는 재료를 쓰는 티켓을 밀어낼 자격이 없다. 0~2개는 그대로 추천하고 그 부족분은 Short 줄과 To buy 원탭 알약이 처리한다(§13.5 ⑨).
...
- **`PaperRing` / `PaperRingArc`(신규 프리미티브).** 12시에서 시계방향으로 도는 **직선 변 폴리라인 띠**. 트랙(`sub` 면)과 값(구간색 면)은 **종이 두 장**이고, 둘 다 평평한 토큰 색 + `PaperGrain` + `paperEdge`(값 띠는 `--paper-edge-onfill`)로 끝난다. **그라데이션·글로우·그림자 없음.** 기본 18각(20°마다 꺾여 지름 156pt에서 변 하나가 27pt, 원 대비 처짐 1.2pt)이고, 정점 반지름 지터는 **한 바퀴에 정확히 맞물리는 저주파 표**다 — 이웃 칸끼리 튀는 표를 쓰면 정점마다 뿔이 서서 톱날이 된다(첫 캡처의 12시 정점이 이웃보다 2.7% 튀었다). 두 가지 규율을 셰이프가 스스로 지킨다: ① **비율 0은 도형을 만들지 않는다**(폭 0 조각의 잉크 점 금지) ② **꽉 찬 고리는 닫힌 서브패스 둘**(바깥 시계·안쪽 반시계, non-zero로 가운데가 뚫린다) — 한 폴리곤으로 이으면 12시에 폭 0의 반지름 슬릿이 윤곽에 남아 `paperEdge` 스트로크가 거기에 실선을 그린다(첫 캡처에서 세로 이음매로 보였다). 잘린 단면은 **부분 호의 양 끝에만** 있다.
- **요일 행 — 오늘만 잉크 솔리드.** 7칸이 균등 폭으로 서고 각 칸은 머리글자(로케일의 `veryShortStandaloneWeekdaySymbols`) + 9각 `PaperBlob` + 그날 먹은 개수(`reffiNum(.body)`)다. 지나간 날 = `paper` 면 + `paperEdge`, **오늘 = 잉크 솔리드 + `PaperGrain` + `paperEdgeOnFill`**(숫자는 `canvas`로 반전), 아직 오지 않은 날 = `sub` 0.55 면에 숫자 없음. **위 카테고리 칩의 "잉크 솔리드 금지"와 충돌하지 않는다**: 그 규칙은 *조작 상태*가 조작 대상보다 무거워지는 것을 막으려는 것이고, 오늘 칸은 조작 상태가 아니라 이 행에서 **어디가 지금인지**를 정하는 유일한 기준점이다(그것 없이는 어느 쪽이 지나간 날이고 어느 쪽이 앞으로 올 날인지 읽히지 않는다) — 탭 알약이 패인 행에서 하는 일과 같다. 게다가 이 행은 눌리지 않아 무거운 표시가 컨트롤로 오독될 여지도 없다.
...
- **`PaperChecklistDialog` = 고르기형(2026-08, 25차).** 같은 문법에 셋을 더한다: **우상단 X**(`PaperCloseButton`,
  ...
  꺼짐 = `paper` 면 + 헤어라인이고 **행 전체가 44pt 타깃**이다(§7.3). **하나도 체크되지 않으면 CTA는 `disabled`**
