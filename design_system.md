...
- **재료 낙하·바운스는 진짜 물리 엔진**(SpriteKit, 레퍼런스 École Vision의 gravity-based). 중력 + `restitution ≈ 0.45`로 떨어져 충돌·통통 튀고, 끌어서 던질 수 있다. 스프링 토큰이 아니라 물리 시뮬레이션 — 재료는 쌓여서 **사라지지 않고 남는다**(§13.6).
...
:root {
  ...
  /* ---- Paper / Glass (§13) ---- */
  --paper-edge:  oklch(25% 0.012 80 / 0.07);   /* 종이 단면 헤어라인(분리 보더 아님) */
  --paper-sheen: oklch(100% 0 0 / 0.22);        /* 리퀴드글래스 상단 시노(버튼) */
...
- **종이 단면(`--paper-edge`)** — 면과 같은 톤의 ~1px 헤어라인을 면 위에 겹쳐 "종이 두께"를 표현. **이는 §6.1의 분리용 보더가 아니다**(면을 나누지 않는 동일 톤 재질 표현). 분리는 여전히 색·여백·그림자로.

### 13.2 리퀴드글래스 (Liquid Glass)
- iOS 26 네이티브 `.glassEffect`(폴백 `.ultraThinMaterial`). **사용처:** 메인 **배경**(컬러 블롭 위 글래스 프로스트), 하단 캡슐 네비, 떠 있는 닫기 버튼. (버튼은 글래스 시노 없이 솔리드+종이질감 — §13.5)
- **메인 배경 = 리퀴드글래스** — 신선도/블루 틴트 블롭을 깔고 글래스로 흐려 프로스트 면을 만든다. 그 위로 재료가 떨어진다.
- **버튼은 글래스 머티리얼로 통째 덮지 않는다** — 색(Blue/sub)을 살리고 상단에 옅은 글래스 **시노**만.
...
**오더 메모 카드(`OrderMemoCard`)** — 주방 오더 티켓: 크림 종이(`ReceiptShape` 상·하 톱니 절취) + 모노 "ORDER" 헤더 + 점선 룰 + 메뉴명/시간 + 재료 체크리스트(신선도) + 옅은 START 스탬프(장식).
