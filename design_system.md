...
| field on canvas (시트 위 입력 칸) | ≥1.2 | 1.11 | 1.20 |
| field on paper (카드 위 입력 칸) | ≥1.2 | 1.18 | 1.19 |
| field on receipt | ≥1.2 | 1.16 | 1.22 |
...
- **`field`는 카드가 아니라 "칸"이라 ≥1.2 하한만 건다.** 카드·CTA 면의 1.35~1.5 구간은 *바탕 위로 뜬 종이 한 장*의 요건인데, 입력 칸은 뜨는 게 아니라 부모 면 안에 앉는다 — 그 구간까지 벌리면 시트 안에 종이가 한 장 더 뜬 것으로 읽힌다. 대신 부모가 캔버스든 종이든 **양쪽 모두**에서 1.19 이상을 지켜야 하고(§2.7 `field` 행), 그래서 값이 두 부모 사이에 앉는다.
