...
- `-toBuy.swipeHint [초]` **밀기 어포던스 힌트 강제** — 목록 맨 윗줄이 20pt 밀렸다 돌아오는 첫 등장 연출을, 설치당 한 번인 `@AppStorage("toBuy.swipeHintSeen")` 플래그와 무관하게 재생한다(단독 지정해도 To buy 탭에 착지 — 다만 **볼 것을 만들려면 `-toBuy.sampleMemo`와 함께** 준다). 뒤에 양수를 붙이면 밀린 채 머무는 시간이 그 값으로 넓어진다 — 기본 0.4초는 XCUITest가 프레임 조회로 잡기엔 너무 좁다(`-fireDismissDelay`와 같은 이유·같은 모양, 파싱 정본은 `ShoppingListContent.swipeHintConfig(from:)`이고 유닛 테스트가 고정한다). **강제 경로는 플래그를 쓰지 않는다** — 같은 인자를 몇 번이든 다시 쓸 수 있다. 반대로 "플래그가 서 있으면 안 뜬다"를 재현할 때는 UserDefaults 인자 `-toBuy.swipeHintSeen YES`를 쓴다. 모션 축소가 켜져 있으면 강제해도 재생하지 않는다(§7.4 — 그리고 그 경우 플래그도 남기지 않아, 나중에 축소를 끄면 힌트가 그때 처음 뜬다). ⚠️ `-toBuy.search`와 **함께 주지 않는다** — 검색 시트가 힌트를 덮는다(코드도 시트가 올라오면 재생을 접는다)
