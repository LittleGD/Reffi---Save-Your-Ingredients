...
- `-analyticsOff` 행동 계측(`Analytics`) 킬스위치 — 큐·업로드 전부 끈다(유닛 테스트 호스트는 인자 없이도 꺼진다). DEBUG 빌드는 켜져 있어도 `channel = 'debug'`로 올라가 서버 뷰가 집계에서 뺀다(`docs/ANALYTICS.md`)
...
- `-history.chipHintForce` 이번 주 히어로의 칩 캡션 힌트("A chip a day. Green is what you ate.", 31차부터 닫기 가능)를 **설치당 한 번**인 `@AppStorage("history.chipHintDismissed")`와 무관하게 강제로 보여준다(`-toBuy.swipeHint` 강제 인자와 같은 결 — 이미 닫은 설치에서도 QA가 다시 볼 수 있다). 반대로 "닫힌 채로 재현"은 UserDefaults 인자 `-history.chipHintDismissed YES`를 쓴다(`-fridge.compact YES` 선례)
...
- `-toBuy.swipeHint [초]` **밀기 어포던스 힌트 강제** — 목록 맨 윗줄이 20pt 밀렸다 돌아오는 첫 등장 연출을, **실행(런치)당 한 번**인 프로세스 스코프 플래그와 무관하게 재생한다(단독 지정해도 To buy 탭에 착지 — 다만 **볼 것을 만들려면 `-toBuy.sampleMemo`와 함께** 준다). 이 플래그는 52차부터 `@MainActor static var`다 — 그 전엔 `@AppStorage("toBuy.swipeHintSeen")`로 설치당 한 번이라 실사용 기기에서 한 번 재생된 뒤로는 다시 볼 길이 없었다(사용자 리포트로 개편). 뒤에 양수를 붙이면 밀린 채 머무는 시간이 그 값으로 넓어진다 — 기본 0.4초는 XCUITest가 프레임 조회로 잡기엔 너무 좁다(`-fireDismissDelay`와 같은 이유·같은 모양, 파싱 정본은 `ShoppingListContent.swipeHintConfig(from:)`이고 유닛 테스트가 고정한다). **강제 경로는 플래그를 쓰지 않는다** — 같은 인자를 몇 번이든 다시 쓸 수 있다. 반대로 "이미 봤으면 안 뜬다"를 재현할 때는 `-toBuy.swipeHintSeen`(값 불필요 — 존재 자체가 신호, 파싱 정본은 `ShoppingListContent.swipeHintSeenAtLaunch(in:)`)을 쓴다 — 이 실행을 처음부터 이미 본 상태로 세울 뿐이라, 이 인자를 안 준 다음 런치는 다시 힌트를 볼 수 있다. 모션 축소가 켜져 있으면 강제해도 재생하지 않는다(§7.4 — 그리고 그 경우 플래그도 남기지 않아, 나중에 축소를 끄면 힌트가 그때 처음 뜬다). ⚠️ `-toBuy.search`와 **함께 주지 않는다** — 검색 시트가 힌트를 덮는다(코드도 시트가 올라오면 재생을 접는다)
