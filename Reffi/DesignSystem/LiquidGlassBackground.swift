...
struct LiquidGlassBackground: View {
    ...
    var body: some View {
        ZStack {
            ReffiColor.canvas
            Circle().fill(accent.opacity(0.55)).frame(width: 300, height: 300).blur(radius: 80)
                .offset(x: -130, y: -180)
            Circle().fill(ReffiColor.blue.opacity(0.3)).frame(width: 260, height: 260).blur(radius: 90)
                .offset(x: 140, y: 60)
            Circle().fill((accentDeep ?? accent).opacity(0.16)).frame(width: 220, height: 220).blur(radius: 80)
                .offset(x: 70, y: 300)
            glassFrost
