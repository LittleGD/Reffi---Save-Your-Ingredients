...
enum ReffiColor {
    ...
    /// neutral-200 · 서브 면 — L .935/.008/85 · D .30/.008/80
    static let sub    = dynamic(light: (0.935, 0.008, 85), dark: (0.30, 0.008, 80))
    ...
    /// 밝은 종이 면 — 뱃지·티켓·글리프 타일. L .99/.006/90 · D .285/.008/82
    static let paper     = dynamic(light: (0.99, 0.006, 90), dark: (0.285, 0.008, 82))
    /// 따뜻한 주방 패스 종이 — 캐러셀 배경. L .95/.016/90 · D .255/.010/84
    static let paperPass = dynamic(light: (0.95, 0.016, 90), dark: (0.255, 0.010, 84))
    /// "흰 영수증" 면의 정본 — 영수증 시트·행. L .985/.004/90 · D .29/.007/83
    /// (인라인 `oklch(0.985, 0.004, 90)`으로 흩어져 있던 값을 토큰화)
    static let receipt   = dynamic(light: (0.985, 0.004, 90), dark: (0.29, 0.007, 83))
    ...
    /// 잉크 토스트 캡슐 면 — 양 모드 모두 어두운 면. L ink(.25/.012/80) · D .33/.010/80
    /// (위 콘텐츠는 양 모드 모두 고정 흰색 — `onInk`가 아니다)
    static let toast      = dynamic(light: (0.25, 0.012, 80), dark: (0.33, 0.010, 80))
