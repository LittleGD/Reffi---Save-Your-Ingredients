...
enum ReffiColor {
    ...
    /// **blue 면 위 보조 줄**(50차) — 제목(`onAccent` 흰색)과 같은 면에 서는 둘째 줄이
    /// 위계를 양보하는 잉크. 홈 CTA의 "Saves N expiring today"가 유일한 소비처다.
    ///
    /// **라이트에서만 파랗고 다크에서는 흰색이다 — 이건 미완성이 아니라 측정 결과다.**
    /// 위 `onAccent` 주석이 적어 둔 대로 `blue`의 다크 L 상한 .565는 "흰 글자로 4.5:1"에서
    /// 역산된 값이라, 다크 면 위에서는 **흰색 4.64:1이 곧 천장**이다. 실측(OKLCH→sRGB→WCAG):
    /// 파랑 쪽으로 아무리 조금 틴트해도 4.5 아래로 떨어지고(`blueLight` 다크값은 2.93:1),
    /// 반대로 어둡게 가도 마찬가지다(중간 톤 면이라 양방향 모두 천장이 낮다). 라이트 면은
    /// 여유가 있어 `blueLight` 값이 4.59:1로 통과한다 — 그래서 그쪽에만 틴트를 준다.
    ///
    /// 다크에서도 파랗게 하려면 **면을 어둡게** 해야 하는데(`blue`의 다크 L 하향), 그건 앱의
    /// 모든 파랑 면이 함께 움직이는 별건이다. 여기서 리터럴로 우회하지 말 것.
    static let onAccentSoft = dynamic(light: (0.93, 0.045, 250), dark: (1, 0, 0))
