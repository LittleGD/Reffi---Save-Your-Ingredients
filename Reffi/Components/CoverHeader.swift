...
///
/// - **헤더 행에는 제목과 X만 선다.** **좌측** 타이틀(`.heading`) + 우측 `PaperCloseButton`.
///   행의 자식이 이 둘뿐이라 행 높이가 X의 터치 타깃(`ReffiChrome.tapMin`)으로 고정되고,
///   제목과 X의 세로 중심이 **구조적으로** 맞는다 — 제목이 두 줄로 접혀도 둘의 중심은 같다.
///
...
/// - **부제(`.caption`)도 제목과 같은 좌측선에 선다**(49차) — 행이 아니라 행 아래 전폭이다.
///   **다시 행 안으로 넣지 말 것.** 행 안에 두면 두 가지가
///   한꺼번에 무너진다: ① 44pt X가 '제목+부제 블록' 전체의 세로 중심에 맞춰져 제목 옆이 아니라 두 줄
///   *사이*에 뜬다(부제 2줄이면 제목 중심보다 24.5pt 아래) ② 부제가 우측 X 44 + 간격에 눌린 좁은
///   컬럼(393pt 화면에서 309pt)에 갇힌다. 전폭(361pt)으로 옮긴다고 한 줄이 되지는 않는다 — 영문 원문도
...
struct CoverHeader<Accessory: View>: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: ReffiSpace.s1) {
            // 이 행의 자식은 **제목과 X 둘뿐이어야 한다.** 부제를 여기 되돌리는 순간 행 높이가
