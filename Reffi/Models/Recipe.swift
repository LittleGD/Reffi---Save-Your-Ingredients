...
struct Recipe: Identifiable, Codable, Equatable {
    ...
    /// 김밥 그 자체인데, 재료에서 파생하면 첫 재료(김·계란)가 대표로 올라와 축약 티켓 146pt에서
    /// 메뉴를 못 읽는다(§13.5 축약 = 아이콘 + 메뉴명뿐 — 아이콘이 곧 메뉴 식별자다).
    var glyph: FoodGlyph {
    ...
    /// 표는 `FoodGlyph.dishKeywords` **한 곳**에만 둔다 — 재료명 경로(`FoodGlyph.match`)와 레시피명
    /// 경로가 같은 목록·같은 가드를 쓰지 않으면 "김밥"이 어디서 들어오느냐에 따라 그림이 갈린다.
