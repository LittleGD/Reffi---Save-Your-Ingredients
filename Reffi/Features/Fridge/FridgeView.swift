...
struct FridgeView: View {
    ...
    /// **스택 첫 장(`stackList`의 i=0) 전용 상단 여유**(63차, 오너 지적 — "영수증 끝에 잘리지
    /// 않게"). 59차 이전엔 `controlRow`가 스크롤 콘텐츠의 첫 줄이었고 그 아래 s5(24) 간격이 회전한
    /// 첫 카드의 윗모서리를 감쌀 여유를 겸했다. 59차가 그 줄을 `fridgeHeader`로 승격해 스크롤 밖으로
    /// 빼면서 `stackList`가 스크롤 콘텐츠의 첫 줄이 됐는데, `ScrollView`는 늘 그래왔듯 콘텐츠 상단에서
    /// 클리핑한다 — 그 경계가 이제 회전한 첫 카드(`tilt(0)`=4°)의 삐져나온 톱니 위 모서리를 직접 자른다.
    ///
    /// 필요한 여유는 i=0의 회전이 카드를 위로 밀어 올리는 양이다: 위 `cardInset` 주석과 축이
    /// 다르다(그쪽은 회전이 카드를 좌우로 미는 양이라 카드 **높이**가 돌고, 이쪽은 카드를 위로
    /// 미는 양이라 카드 **폭**이 돈다) — 폭 334pt(iPhone 17: 402 − 2×(margin 16 + cardInset 18))의
    /// 반, 167 × sin4° ≈ 11.7pt. s3(12)로도 명목상 덮이지만 여유가 0.3pt뿐이라 톱니 시드·글자
    /// 크기별 실측 편차를 흡수하지 못한다 — s4(16)가 안전 마진과 함께 덮는다.
    /// 스페어 심(기본 글자 크기)의 전/후 스크린샷으로 잘림이 사라지는지 확인한다.
    ///
    /// **59차의 "고정 헤더 밑으로 콘텐츠가 비치면 안 된다"는 의도는 그대로 유지된다** — 이 여유는
    /// `fridgeHeader`가 아니라 `stackList` 내부(스크롤 콘텐츠 쪽)에 있으므로 `ScrollView`의 프레임
    /// 자체는 옮기지 않는다. 헤더와 스크롤 뷰는 여전히 겹치지 않는 형제라 콘텐츠가 헤더 뒤로 비치는
    /// 경로는 생기지 않고, 클리핑 경계도 헤더 바로 아래 그대로 남는다 — 그 경계와 첫 카드의 회전
    /// 여백 사이에 숨 쉴 틈 하나가 생겼을 뿐이다.
    private let stackTopClearance: CGFloat = ReffiSpace.s4
    ...
    private func stackList(_ list: ListDigest) -> some View {
        ...
        return VStack(alignment: .leading, spacing: ReffiSpace.s5) {
            ...
            // 위 `stackTopClearance` 선언 주석 — i=0의 회전 여유를 여기서 얹는다(59차가 걷어 간
            // `controlRow`의 몫을 스크롤 콘텐츠 안에서 되찾아, `ScrollView`의 상단 클리핑이 첫
            // 카드의 톱니를 다시 자르지 않게 한다).
            .padding(.top, stackTopClearance)
    ...
    private var titleRow: some View {
        // 한국어와 영어 화면 제목 모두 OK단단체 Display를 사용한다(§3.1).
...
struct ExpandedFridgeCard: View {
    ...
    /// 근거가 사라졌지만 당시엔 값을 바꾸지 않았다: 위쪽 상한(카드가 뷰포트를 넘겨 영수증이
    /// 잘린 채 서는 지점 — 기본 글자 크기 iPhone 17 실측, 120이면 55pt 초과)이 도장과 무관하게
    /// 유효했고, 이 화면의 다른 실측도 당시 값 88을 전제로 잡혀 있었다(아래 63차 문단이 그
    /// 전제를 자체 스크롤로 풀었다).
    ///
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            //    자라 카드 높이를 그만큼 먹는데, 판정 버튼·덱이 스크롤 밖에 도킹된 이 화면에서
            //    행 높이는 곧 스크롤 없이 보이는 명세 줄 수다(88 시절 실측 "몫 고정, 편집을 행에서
            //    빼야 Storage 행까지"는 63차-b의 영수증 자체 스크롤로 전제가 풀렸지만, 스크롤을
            //    덜 시키는 쪽이 여전히 낫다). 히트 영역은 오버레이에서 그대로 44를 지킨다 —
            //    줄이는 게 아니라 겹치는 것이다.
            //
