        ...
        return VStack(alignment: .leading, spacing: ReffiSpace.s3) {
            ...
            if !suggested.isEmpty {
                ...
                Text("Ran out, based on what you use often")
                    .reffiType(.caption).foregroundStyle(ReffiColor.ink2)
                ForEach(suggested, id: \.name) { row($0) }
            }
        }
...
private struct ToBuySearchSheet: View {
    ...
    /// FREQUENT(빨리 담기 단축키) → 사전 전체를 카테고리로 묶은 섹션(`IngredientLexicon.categoryOrder`).
    ...
    private func tile(_ item: GridTile, listed: Bool) -> some View {
        Button {
            ...
            .background {
                // 종이 시드는 `stableHash` — `String.hashValue`는 런치마다 시드가 바뀌어 같은 타일이
                // 매번 다른 종이결로 뜨고 스크린샷 회귀가 불가능해진다(DishGlyphCatalog와 같은 이유).
                let s = PaperRect(cornerRadius: ReffiRadius.md,
                                  seed: Int(DishGlyphCatalog.stableHash(item.key) % 4))
